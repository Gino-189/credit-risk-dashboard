...
## Student Name: Ginojan Kanagarajah
...
# Frame 1 (Control Panel + Grid + Drill-down)
# Sidebar = control panel (filters)
# Main area = KPIs + charts
...
# KPIs (Task 5 — Descriptive Statistics)
...
top_category = filtered_df["purpose_group"].mode()[0] if len(filtered_df) > 0 else "N/A"
...
st.markdown(
    "Filters dynamically refine the dataset, allowing comparison of credit behaviour across loan types, age groups, and risk categories."
)

st.subheader("Data Quality & Preprocessing Impact")

col1, col2 = st.columns(2)

col1.metric("Original Mean Credit", round(df_clean["credit_amount"].mean(), 2))
col2.metric("Capped Mean Credit", round(df_clean["credit_amount_capped"].mean(), 2))

st.markdown("Outlier capping reduces the influence of extreme values, improving reliability of averages.")
...
# Credit by Class
fig, ax = plt.subplots()
filtered_df.boxplot(column="credit_used", by="class", ax=ax)
ax.set_title("Credit Distribution by Risk Category (Higher Spread in Bad Credit)")
...
st.write(f"Showing {len(drill_df)} records after drill-down filter")
st.dataframe(
...
**Mitigation:** This dashboard is designed strictly for exploratory analysis and explicitly avoids automated decision-making. Clear explanations are provided to emphasise that patterns observed are descriptive, not predictive.

**Professional Consideration:**  
Misinterpretation of statistical outputs-such as relying solely on mean values in skewed distributions-can lead to flawed conclusions. Additionally, users may overlook how filtering choices influence results.  
**Mitigation:** The dashboard incorporates multiple statistical views and uses descriptive chart titles to guide interpretation, highlighting that results are sensitive to filters.

The dashboard avoids using sensitive attributes, gender or personal status, to prevent biased interpretations.
""")